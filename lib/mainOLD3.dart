        ...
        builder: (ctx) => Scaffold(
          appBar: AppBar(
            ...
            title: Row(children: [
              ...
              TextButton(onPressed: () => Navigator.push(ctx, MaterialPageRoute(builder: (_) => const TelaQGPLogin())), child: const Text('QGP', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14))),
...
class _TelaLoginState extends State<TelaLogin> {
        ...
        child: Column(
          ...
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Tutorial', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.amber)),
              IconButton(icon: const Icon(Icons.close, color: Colors.white), onPressed: () => Navigator.pop(ctx)),
            ]),
            const Divider(color: Colors.amber),
            const SizedBox(height: 16),
            const Text('COMO ADICIONAR CANDIDATOS', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.amber)),
            const SizedBox(height: 12),
            _itemTutorial('1', 'Acesse o painel Admin em "Gestão de Dados" no menu superior.'),
            _itemTutorial('2', 'Clique no botão "Adicionar Candidato" na aba Candidatos.'),
            _itemTutorial('3', 'O sistema abre o seletor de arquivos do sistema.'),
            _itemTutorial('4', 'Selecione uma IMAGEM do candidato que deve seguir o padrão de nomenclatura abaixo.'),
            const SizedBox(height: 12),
            Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black26, borderRadius: BorderRadius.circular(8)), child: const Column(children: [
              Text('PADRÃO DO NOME DO ARQUIVO:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.amber)),
              SizedBox(height: 8),
              Text('Nome_Número_Partido.jpg/png', style: TextStyle(fontFamily: 'monospace', color: Colors.white)),
              SizedBox(height: 8),
              Text('Exemplo: Maria Silva_45_Partido Popular.jpg', style: TextStyle(fontFamily: 'monospace', color: Colors.green)),
            ])),
            const SizedBox(height: 12),
            _itemTutorial('5', 'O sistema extrai automaticamente: Nome, Número e Partido.'),
            _itemTutorial('6', 'A foto é exibida para confirmação.'),
            _itemTutorial('7', 'Clique em "Injetar Votos" para adicionar votos de teste.'),
            const SizedBox(height: 12),
            const Text('DICAS', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.amber)),
            const SizedBox(height: 8),
            _itemTutorial('•', 'Use fotos no formato JPG ou PNG.'),
            _itemTutorial('•', 'O nome deve conter exatamente 3 partes separadas por underscore (_).'),
            _itemTutorial('•', 'Consulte "Ver Resultados" para ver o gráfico de votação.'),
          ],
        ),
      ),
    ));
  ...
  Widget _itemTutorial(String numero, String texto) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(width: 24, height: 24, decoration: BoxDecoration(color: const Color(0xFF8A2BE2), borderRadius: BorderRadius.circular(12)), child: Center(child: Text(numero, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)))),
    ...
    return Scaffold(
      ...
      body: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [const Color(0xFF8A2BE2), Colors.purple[800]!], begin: Alignment.topCenter, end: Alignment.bottomCenter)), child: SafeArea(child: Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        if (_tipoUsuario == 'eleitor')
          ElevatedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const Tela1Cadastro())), icon: const Icon(Icons.person_add), label: const Text('Cadastre-se', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(minimumSize: const Size(200, 50)))
        else if (_tipoUsuario == 'admin')
          ElevatedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TelaAdminLogin())), icon: const Icon(Icons.admin_panel_settings), label: const Text('Admin', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(minimumSize: const Size(200, 50), backgroundColor: Colors.amber))
        else
          ElevatedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TelaPesquisadorLogin())), icon: const Icon(Icons.search), label: const Text('Pesquisador', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(minimumSize: const Size(200, 50), backgroundColor: Colors.blueGrey)),
      ])))),
    );
  }
}
...
class _TelaAdminLoginState extends State<TelaAdminLogin> {
  ...
  String _gerarSenha() {
    ...
    String s = '';
    s += maiusculas[(random ~/ 100) % maiusculas.length];
    s += minusculas[(random ~/ 10) % minusculas.length];
    s += numeros[(random) % numeros.length];
    s += especiais[(random ~/ 1000) % especiais.length];
    for (int i = 0; i < 2; i++) {
      const todos = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#\$%&*';
      s += todos[(random + i * 7) % todos.length];
    }
    final lista = s.split('');
    lista.shuffle();
    return lista.join();
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Login Admin'), backgroundColor: Colors.amber[700]), body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      ElevatedButton(onPressed: () { if (_senhaController.text == 'admin123') Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const TelaAdmin())); else ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Senha incorreta'))); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, minimumSize: const Size(200, 50)), child: const Text('Entrar', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
      const SizedBox(height: 30),
      const Divider(),
      TextButton.icon(onPressed: () => setState(() => _mostrarGerador = !_mostrarGerador), icon: const Icon(Icons.password), label: Text(_mostrarGerador ? 'Ocultar Gerador' : 'Gerar Nova Senha')),
      if (_mostrarGerador) ...[
        const SizedBox(height: 16),
        const Text('Gerador de Senha (6 caracteres)', style: TextStyle(fontSize: 14)),
        const SizedBox(height: 8),
        ElevatedButton.icon(onPressed: () => setState(() => _senhaGerada = _gerarSenha()), icon: const Icon(Icons.refresh), label: const Text('Gerar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8A2BE2))),
        if (_senhaGerada.isNotEmpty) ...[
          const SizedBox(height: 16),
          Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.grey[800], borderRadius: BorderRadius.circular(8)), child: Column(children: [
            const Text('Nova Senha:', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            SelectableText(_senhaGerada, style: const TextStyle(fontSize: 24, letterSpacing: 2, fontWeight: FontWeight.bold, color: Colors.amber)),
            const SizedBox(height: 8),
            const Text('Copie e use esta senha!\nRequer: 1 maiuscula, 1 minuscula, 1 numero, 1 especial', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: Colors.grey)),
          ])),
        ],
      ],
    ])));
        ...
        children: [
          ...
          Semantics(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Ver Resultados', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          Semantics(
            ...
            button: true,
            child: ElevatedButton.icon(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TelaQGPLogin())),
              icon: const Icon(Icons.pie_chart, color: Colors.white),
              label: const Text('Ver QGP', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
...
Widget _tabCandidatos() {
    final totalVotos = DatabaseService.getTotalVotos();
    return Column(
      children: [
        const Padding(
        ...
        Semantics(
          label: 'total_votos_global',
          child: Text('Total de votos no sistema: $totalVotos', style: const TextStyle(color: Colors.transparent, fontSize: 1)),
        ),
        Row(
          children: [
            ElevatedButton.icon(
        ...
        Row(
          children: [
            Semantics(
              label: 'botao_injetar_votos',
              button: true,
              child: ElevatedButton(onPressed: _injectVotes, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF008F39)), child: const Text('Injetar Votos', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
            ),
            const SizedBox(width: 8),
            Semantics(
              label: 'botao_limpar_votos',
              button: true,
              child: ElevatedButton(onPressed: _clearVotes, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFB24040)), child: const Text('Limpar Votos', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
            ),
          ],
        ),
        const SizedBox(height: 16),
  ...
  Widget _tabTestes() {
    return Padding(padding: const EdgeInsets.all(16), child: Column(children: [const Text('GERADORES', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 20),
      ElevatedButton.icon(onPressed: _gerarEleitores, icon: const Icon(Icons.people, color: Colors.white), label: const Text('Gerar Eleitores', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8A2BE2))),
      const SizedBox(height: 16),
      ElevatedButton.icon(onPressed: _gerarVotos, icon: const Icon(Icons.how_to_vote, color: Colors.white), label: const Text('Gerar Votos', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF008F39))),
      const SizedBox(height: 16),
      ElevatedButton.icon(onPressed: _limpar, icon: const Icon(Icons.delete_forever, color: Colors.white), label: const Text('Limpar Tudo', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFB24040)))]));
  }
...
class _TelaResultadosState extends State<TelaResultados> {
  ...
  Widget build(BuildContext context) {
    ...
    String dadosAuditoria = "AUDITORIA:${t}";
    for (var cand in c) {
      dadosAuditoria += "|${cand.nome}:${cand.numero}:${cand.votos}:${(t > 0 ? (cand.votos / t * 100).round() : 0)}%";
    }
    ...
    return Scaffold(appBar: AppBar(title: const Text('Resultados'), backgroundColor: const Color(0xFF0D47A1), foregroundColor: Colors.white), body: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
      ...
      Offstage(
        offstage: true,
        child: Semantics(
          label: 'dados_auditoria',
          child: Text(dadosAuditoria, style: const TextStyle(color: Colors.transparent, fontSize: 1)),
        ),
      ),
      const SizedBox(height: 20),
...
class TelaQGPLogin extends StatefulWidget {
  const TelaQGPLogin({super.key});
  @override
  State<TelaQGPLogin> createState() => _TelaQGPLoginState();
}

class _TelaQGPLoginState extends State<TelaQGPLogin> {
  final _senhaController = TextEditingController();
  static const _senha = 'qgp123';

  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Login QGP'), backgroundColor: const Color(0xFF8A2BE2)), body: Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Text('Digite a senha', style: TextStyle(fontSize: 18)), const SizedBox(height: 16),
      TextField(controller: _senhaController, obscureText: true, decoration: const InputDecoration(labelText: 'Senha', border: OutlineInputBorder())), const SizedBox(height: 16),
      ElevatedButton(onPressed: () { if (_senhaController.text == _senha) Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const TelaQGP())); else ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Senha incorreta'))); }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF8A2BE2), minimumSize: const Size(200, 50)), child: const Text('Entrar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
    ])));
  }
}
...
class TelaPesquisadorLogin extends StatefulWidget {
  const TelaPesquisadorLogin({super.key});
  @override
  State<TelaPesquisadorLogin> createState() => _TelaPesquisadorLoginState();
}

class _TelaPesquisadorLoginState extends State<TelaPesquisadorLogin> {
  final _senhaController = TextEditingController();
  static const _senha = 'pesq123';

  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Login Pesquisador'), backgroundColor: Colors.blueGrey[700]), body: Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Text('Digite a senha', style: TextStyle(fontSize: 18)), const SizedBox(height: 16),
      TextField(controller: _senhaController, obscureText: true, decoration: const InputDecoration(labelText: 'Senha', border: OutlineInputBorder())), const SizedBox(height: 16),
      ElevatedButton(onPressed: () { if (_senhaController.text == _senha) Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const TelaPesquisador())); else ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Senha incorreta'))); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey, minimumSize: const Size(200, 50)), child: const Text('Entrar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
    ])));
  }
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Pesquisador'), backgroundColor: Colors.blueGrey[700], foregroundColor: Colors.white), body: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
        ElevatedButton.icon(onPressed: () => _registrar('vermelho'), icon: const Icon(Icons.thumb_down, color: Colors.white), label: const Text('Outro', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFB24040), minimumSize: const Size(100, 50))),
        ElevatedButton.icon(onPressed: () => _registrar('verde'), icon: const Icon(Icons.thumb_up, color: Colors.white), label: const Text('Favor', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF008F39), minimumSize: const Size(100, 50))),
        ElevatedButton.icon(onPressed: () => _registrar('branco'), icon: const Icon(Icons.question_mark, color: Colors.white), label: const Text('Indec', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: Colors.grey, minimumSize: const Size(100, 50))),
      ]),
    ])));
  }
